...
class Repair_record(db.Model):
    ...
    def to_json(self):
        ...
        if self.production_id:
            production_name = self.repair_records.name
        json_repair_record = {
            'id': self.id,
...
class Production(db.Model):
    ...
    def to_json(self):
        ...
        if self.customer_id:
            customer_name = self.bought.customer_name
        ...
        json_production = {
            ...
            'iids': [{'iid_id':iid.iids.id, 'iid_name':iid.iids.item, 'iid_product_id':iid.product_id, 'iid_serial_number':iid.serial_number} for iid in iids]
        }
...
class Pt_i(db.Model):
    ...
    identifier_id = db.Column(db.Integer, db.ForeignKey('identifiers.id'))
    product_type_id = db.Column(db.Integer, db.ForeignKey('product_types.id'))
    ...
    def to_json(self):
        ...
        if self.identifier_id:
            identifier_name = self.i_pt.item
        if self.product_type_id:
            product_type_name = self.pt_i.name
        json_pt_i = {
            'id': self.id,
...
class Product_type(db.Model):
    ...
    def to_json(self):
        ...
        json_product_type = {
            ...
            'pt_is': [{'id':pt_i.id,'name':pt_i.i_pt.item} for pt_i in pt_is ]
...
class Iid(db.Model):
    ...
    def to_json(self):
        ...
        if self.identifier_id:
            identifier_name = self.iids.item
            identifier_type = None
            if self.iids.identifier_type:
                identifier_type = self.iids.identifier_type.name
        if self.production_id:
            production_name = self.pids.name
            production_type = None
            if self.pids.product_type:
                production_type = self.pids.product_type.name
        json_iid= {
            'id': self.id,
...
class Identifier(db.Model):
    ...
    identifier_type_id = db.Column(db.Integer, db.ForeignKey('identifier_types.id'))
    ...
    def to_json(self):
        ...
        if self.identifier_type_id:
            identifier_type_name = self.identifier_type.name
        if self.iids:
            iids = self.iids
        if self.pt_is:
            pt_is = self.pt_is
        json_identifier  = {
            ...
            'pt_is':[{'id':pt_i.id,'name':pt_i.pt_i.name} for pt_i in pt_is]
...
class Customer(db.Model):
    ...
    def to_json(self):
        ...
        productions = []
        if self.adress_id:
            adress_name = self.customer.name
            adress_adcode = self.customer.adcode
            f_adress_adcode = self.customer.adcode - self.customer.adcode%10000
...
class Change_record(db.Model):
    ...
    change_userid = db.Column(db.Integer, db.ForeignKey('users.id'))
    ...
    def to_json(self):
        ...
        if self.change_userid:
            change_userid = self.change_records.username
        json_change_record  = {
            'id': self.id,
